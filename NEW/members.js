...
let q = "";
  ...
  function normalizeMember(doc) {
    ...
    const activeTime = (d.activeTime || d.projectTime || '').toString().trim();
...
// ===== Render chips =====
  ...
  function memberCard(member) {
    ...
    body.innerHTML = `
      ...
      ${member.bio ? `<div class="mcard-bio">${escapeHtml(member.bio)}</div>` : ""}
      ${member.achievements ? `<div class="mcard-achv">${escapeHtml(member.achievements)}</div>` : ""}
      ...
      <div class="mcard-actions">
        ...
        <button type="button" class="mcard-btn ghost" data-act="social"><span>↗</span> Social</button>
      </div>
    `;
    ...
    body.querySelector('[data-act="social"]').addEventListener("click", (e) => {
  ...
  function openModal(member) {
    ...
    const fb = member.links.facebook
      ? `<a href="${escapeHtml(member.links.facebook)}" target="_blank" rel="noopener">${escapeHtml(member.links.facebook)}</a>`
      : "<span style='opacity:.7'>(không có)</span>";
    const ig = member.links.instagram
      ? `<a href="${escapeHtml(member.links.instagram)}" target="_blank" rel="noopener">${escapeHtml(member.links.instagram)}</a>`
      : "<span style='opacity:.7'>(không có)</span>";
      ...
      <div class="profile-sheet">
        ...
        <div class="p-right">
          ...
          <div class="p-info">
            ...
            <div class="p-value">${member.achievements ? escapeHtml(member.achievements) : "<span style='opacity:.7'>(chưa cập nhật)</span>"}</div>
          ...
          <div class="p-quote">“Cảm ơn bạn đã ghé xem team tụi mình.”</div>
