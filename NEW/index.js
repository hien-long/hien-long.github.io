...
/* ===== Inline Script 5 ===== */
/* Theme chooser popup: show on first visit, pick theme then auto-close */
(function(){
  const STORAGE_KEY = "tb_theme";
  const overlay = document.getElementById("theme-chooser-overlay");
  const btnLight = document.getElementById("theme-choice-light");
  const btnDark  = document.getElementById("theme-choice-dark");

  function normalizeTheme(key){
    if (!key) return null;
    if (key === "grey" || key === "dark") return "dim-dark";
    if (key !== "light" && key !== "dim-dark") return null;
    return key;
  }

  function applyTheme(key){
    const k = (key === "light") ? "light" : "dim-dark";
    document.body.setAttribute("data-theme", k);
    const btn = document.getElementById("ms-theme-btn");
  ...
  document.addEventListener("DOMContentLoaded", function(){
    // If user already has theme saved, don't show popup.
    const saved = normalizeTheme(localStorage.getItem(STORAGE_KEY));
    if (!saved){
      // Ensure a sane default behind the modal so text is readable
      applyTheme("dim-dark");
      open();
    }
  });
  ...
  // Click outside closes ONLY if already chose before (avoid accidental close on first-time)
  if (overlay){
